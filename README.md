...
3. Excute `analyseplugin <path to LADSPA plugin>/ladspa_dsp.so` to determine plugin name and label (here: n.  
3. Run `pacmd load-module module-ladspa-sink sink_name=ladspa_out master=<master_sink> plugin=<plugin name> label=<plugin label>`.
4. Select new LADSPA sink as system sink (Ubuntu 18.04 Desktop: Settings > Sound > Output > LADSPA_Plugin <plugin label>  on <master sink selected above>).

Example:

	pacmd list sinks
	analyseplugin /usr/local/lib/ladspa/ladspa_dsp.so
	pacmd load-module module-ladspa-sink sink_name=ladspa_out master=alsa_output.pci-0000_00_14.2.analog-stereo plugin=ladspa_dsp label=ladspa_dsp
